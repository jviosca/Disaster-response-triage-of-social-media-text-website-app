...
def build_model():
    """ Function to build a logistic regression classifier of text messages on 36 categories or labels
    
    GridSearch is used to optimize for the use of balanced class weights vs not using class weight balance
    In balanced class weight, weights are inverserly proportional to class frequencies in input data as:
        n_samples / (n_classes * np.bincount(y))) 
    In class weight = None, all classes are supposed to have weight = 1
    
    f1_macro is used as scorer as it is more sensitive to the score of the less frequent class, which in this use case 
...
def main():
    """ Function to run the entire machine lerning text classification pipeline, from loading the dataset, training the classifier to exporting the model as a pickle file
    """
